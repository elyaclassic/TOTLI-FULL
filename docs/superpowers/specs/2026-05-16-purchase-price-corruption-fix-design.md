...
7c. **Konvergensiya fence (C2):** kg/unit≠1.0 (400gr uslubli) retsept uchun backfill `new` == mustaqil hisoblangan jonli `cost_per_unit` (`_calculate_total_material_cost/output_units` mantiqi). EMPIRIK ISBOT (8 real SKU, jonli DB): `_own_cost_per_kg(r)*recipe_kg_per_unit(r)` == jonli `cost_per_unit` AYNAN (IRIS 400gr 14,047.7==14,047.7; KITOB1 64,297.9). Yakuniy review "C1 divergensiya" — XATO derivatsiya; backfill formulasi to'g'ri. Bu test nizoni doimiy yopadi.
...
- **I1 (hardening, bloklamaydi):** PRC-/BACKFILL- `_next_doc` SELECT-max→INSERT — bir vaqtda ikki production-confirm (alohida sessiya) bir xil max o'qib UNIQUE IntegrityError berishi mumkin (bittasi rollback). Single-worker uvicorn'da ehtimol past; eski `production.number` sxemasi KAFOLATLI fail edi — bu yaxshilanish. Ideal hardening: INSERT atrofida retry-on-IntegrityError. Alohida kuzatiladi, bu deploy'ni bloklamaydi.
- **C1 (yakuniy review) QAT'IY RAD ETILDI:** `production_output_quantity_for_stock` (production_order.py:205-219) — output birligi "dona" → `production.quantity` (jami 130 SKU); aks holda ×kgpu. Divergensiya FAQAT (kg-birlik & kgpu≠1.0) da bo'lardi. **Butun jonli populyatsiya skani: 168 dan 0 ta shunday** (130 dona + 38 kg&kgpu=1.0 → hammasida `_own_cost_per_kg(r)*kgpu` == jonli `cost_per_unit` AYNAN). Churn yo'q, formula to'g'ri, o'zgarmaydi. Yakuniy reviewer sintetik test mahsulotга birlik bermagan (real'da 0 ta) → soxta fail.
